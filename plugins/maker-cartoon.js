...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (m.sender in conn.cartoon)
    throw "There is still an unfinished process. Please wait.";
  let q = m.quoted ? m.quoted : m;
  let mime = (q.msg || q).mimetype || q.mediaType || "";
  if (!mime) throw `Where is the picture you want to convert to a cartoon?`;
  if (!/image\/(jpe?g|png)/.test(mime)) throw `File type ${mime} not supported.`;
  ...
  try {
    Cartoon(img).then(async (response) => {
      if (response.message === "success") {
        ...
        let sticker = new Sticker(response.download.head, {
        ...
        m.reply(
          "The picture does not reveal a face. Please send one with a visible face."
        );
      }
    });
  } catch {
